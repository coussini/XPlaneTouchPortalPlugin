...
def server_program():
    ...
    server_socket.bind((host, port))  # Server creating listening socket. Bind host address and port together (takes a tuple as argument)
    ...
    client_connection, client_address = server_socket.accept() # (<<BLOCKS>> execution and waits for an incoming connection)
    while True:
        data = client_connection.recv(1024).decode() # (waiting until a client send a data packet to the server. Won't accept data packet greater than 1024 bytes) 
        if not data:
